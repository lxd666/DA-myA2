function [Acc,Res_performance] = MEDA(Xs,Ys,Xt,Yt,options)
...
knn_model = fitcknn(X(:,1:n)',Ys,'NumNeighbors',n_neighbors);
...
%% 自己加
clab=[1,2]; %[正类，负类]
Res_performance = performance(Yt,Yt_pred,[],clab);  %自己添加,w(:,2)判断为ASD正类的概率
end
